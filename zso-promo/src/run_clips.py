...
def run(s):
    sid = s[0]
    try:
        return video(STYLE + MOTION[sid], f"clips/{sid}.mp4",
                     first_frame=f"key/{sid}.png", seconds=8, aspect="16:9", negative=NEG)
...
print("HOTOVO", len(os.listdir("clips")), "klipů")
